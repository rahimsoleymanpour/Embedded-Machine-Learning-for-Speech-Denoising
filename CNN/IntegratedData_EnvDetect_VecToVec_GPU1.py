...
# Path('./ResultMatData/Test(SNR'+str(SNR)+')_'+TestSpkSet[0]).mkdir(parents=True, exist_ok=True)  # create the path for clean data


# TestNoisySig=scipy.io.loadmat('/storage/mso257/SpeechIntelligibility/Data/1_complete.mat')
# TestNoisyDataEnvLPF1= TestNoisySig["Data_env_lpf1"].reshape((len(TestNoisySig["Data_env_lpf1"])), n_steps, n_features)
# Prediction=model.predict(TestNoisyDataEnvLPF1)
# Prediction= Prediction.reshape((len(TestNoisySig["Data_env_lpf1"]), n_features)).T

# #Save file mat
# scipy.io.savemat('./ResultMatData/Test(SNR'+str(SNR)+')_'+TestSpkSet[0]+'/1_complete_1.mat', mdict={'Prediction': Prediction})
